...
const RRReportProcessor = () => {
  ...
  const exportToExcel = () => {
    ...
    // Prepare data for export with enhanced formatting indicators
    const exportData = [
      // Title row
      [`📊 RR Report Dashboard - Generated on ${new Date().toLocaleDateString()} at ${new Date().toLocaleTimeString()}`],
      [''], // Empty row
      // Headers with emojis for better visual separation
      [
        '🏠 Unit Code', '🏗️ Unit Type', '📝 Description', '🏢 Rental Type', '📅 Vacant As Of', '🚪 Vacate Type',
        '📆 Future Move In', '🔧 Work Order', '💰 Asking Rent', '📋 Make Ready Notes', '⏰ Est. Ready Date',
        '✅ Rent Ready', '📅 Actual Ready', '🔢 Job Code', '💬 Comments', '🏘️ Property', '🏷️ Category', '📊 Status',
        '⏳ Days Until Ready', '⚠️ Has Issues'
      ],
      ...
      ...filteredData.map(unit => [
        ...
        // Enhanced Rent Ready with visual indicator
        unit.rentReady === 'yes' ? '✅ YES' : '❌ NO',
        ...
        unit.jobCode,
        unit.comments,
        // Property with visual indicator
        `🏘️ ${unit.property}`,
        // Category with status emoji
        `${getCategoryEmoji(unit.category)} ${unit.category}`,
        unit.status,
        // Days with visual indicator
        unit.daysUntilReady !== null ? 
          (unit.daysUntilReady <= 0 ? '🟢 Ready' : 
           unit.daysUntilReady <= 30 ? `🔵 ${unit.daysUntilReady}d` : 
           `⚪ ${unit.daysUntilReady}d`) : '—',
        // Has Issues with clear indicator
    ...
    const columnWidths = [
      { wch: 15 }, // Unit Code
      { wch: 18 }, // Unit Type
      { wch: 30 }, // Unit Description
      { wch: 18 }, // Rental Type
      { wch: 15 }, // Vacant As Of
      { wch: 15 }, // Vacate Type
      { wch: 18 }, // Future Move In Date
      { wch: 15 }, // Work Order
      { wch: 15 }, // Asking Rent
      { wch: 25 }, // Make Ready Notes
      { wch: 18 }, // Estimated Ready Date
      { wch: 15 }, // Rent Ready
      { wch: 18 }, // Actual Ready Date
      { wch: 15 }, // Job Code
      { wch: 35 }, // Comments
      { wch: 15 }, // Property
      { wch: 35 }, // Category
      { wch: 15 }, // Status
      { wch: 18 }, // Days Until Ready
      { wch: 12 }  // Has Issues
    ];
    ...
    const headerLength = exportData[2].length;
    worksheet['!merges'] = [
      { s: { r: 0, c: 0 }, e: { r: 0, c: headerLength - 1 } }
    ];

    // Set up auto-filter on headers
    worksheet['!autofilter'] = { ref: XLSX.utils.encode_range({
      s: { c: 0, r: 2 },
      e: { c: headerLength - 1, r: 2 + filteredData.length }
    })};
    ...
    const summaryData = [
      ...
      [`📋 Total Units Processed: ${cleanedData.length}`],
      [`🎯 Units Shown (After Filters): ${filteredData.length}`],
      [`⚠️ Flagged Units: ${filteredData.filter(u => u.hasIssues).length}`],
      [''],
      ...
      [''],
      ['🚨 FLAGGED UNITS BREAKDOWN'],
      ['Issue Type', 'Count'],
      [`Ready = "Yes" but no actual date`, filteredData.filter(u => u.rentReady === 'yes' && (!u.actualReadyDate || u.actualReadyDate === '')).length],
      [`Rented but not marked ready`, filteredData.filter(u => u.futureMoveInDate && u.futureMoveInDate.toString().trim() !== '' && u.rentReady !== 'yes').length],
      [`Development but marked ready`, filteredData.filter(u => u.category === 'Down/Hold/Model/Development' && u.rentReady === 'yes').length],
      [''],
      ['📈 AVAILABILITY BREAKDOWN'],
      [`🟢 Ready Now`, filteredData.filter(u => u.category === 'Available & Rent Ready').length],
      [`🟡 Ready (Flagged)`, filteredData.filter(u => u.category === 'Available & Rent Ready (Flagged)').length],
      [`🔵 Ready in 30 Days`, filteredData.filter(u => u.category === 'Available in Next 30 Days').length],
      [`🟣 Ready in 31-60 Days`, filteredData.filter(u => u.category === 'Available in Next 31-60 Days').length],
      [`⚪ Ready in 60+ Days`, filteredData.filter(u => u.category === 'Available in More than 60 Days').length],
      [`🟠 Already Rented`, filteredData.filter(u => u.category === 'Already Rented').length],
      [`🔴 Down/Hold/Development`, filteredData.filter(u => u.category === 'Down/Hold/Model/Development').length]
    ];
    ...
    // Create a "Flagged Units Only" sheet for easy review
    const flaggedUnits = filteredData.filter(u => u.hasIssues);
    if (flaggedUnits.length > 0) {
      const flaggedData = [
        ['🚨 FLAGGED UNITS - DATA QUALITY ISSUES'],
        [''],
        ['🏠 Unit', '🏘️ Property', '🏷️ Category', '✅ Rent Ready', '📅 Actual Ready', '💬 Issue Description'],
        ...flaggedUnits.map(unit => [
          unit.unitCode,
          unit.property,
          unit.category,
          unit.rentReady === 'yes' ? '✅ YES' : '❌ NO',
          unit.actualReadyDate || '❌ MISSING',
          getIssueDescription(unit)
        ])
      ];

      const flaggedSheet = XLSX.utils.aoa_to_sheet(flaggedData);
      flaggedSheet['!cols'] = [
        { wch: 12 }, { wch: 12 }, { wch: 30 }, { wch: 15 }, { wch: 15 }, { wch: 50 }
      ];
      flaggedSheet['!merges'] = [{ s: { r: 0, c: 0 }, e: { r: 0, c: 5 } }];
      
      XLSX.utils.book_append_sheet(workbook, flaggedSheet, '🚨 Flagged Units');
    }
